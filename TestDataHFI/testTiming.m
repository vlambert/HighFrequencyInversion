...
%     end
    ...
    if i ==1
       mmtmpAll = zeros(length(fspace),nDiv,ns,length(Lambdas)); 
...
figure(10);clf
figure(11);clf
set(gcf,'Position',[-1100 1197 1122 656])
factor = 0.1;
for lambin = 1:length(Lambdas)
    figure(10)
...
for lambin = 1:length(Lambdas)
    ...
    for di = 1:nDiv
        h3=figure(3);clf;
        set(gcf,'Position',[-1026 1107 915 748]);
        hold on
        mmtallsum = zeros(length(t),1);
        for si = 1:nSu
            mmtall = (ifft(mmtmpAll(:,di,subevents(si),lambin),nfft,'symmetric')); 
            mmtall = mmtall(1:nt);
            mmtallsum = mmtallsum+mmtall;
            [u,d]=envelope(real(mmtall));
            [peaks,locs,w,p] = findpeaks(u,t,'NPeaks',3,'SortStr','descend','MinPeakDistance',5);
            subplot(nSu,1,si)
            plot(t,mmtall,'k','LineWidth',2); hold on;
            plot(t,u,'r','LineWidth',2)
            xlim([min(t) min(max(t),15)])
            box on; 
            tmy = ylim;
            for j = 1:length(peaks)
                text(locs(j),1.02*tmy(2),sprintf('%.2f',locs(j)),'FontSize',14);
            end
        end
%         subplot(nSu+1,1,nSu+1)
%         plot(t,mmtallsum,'k','LineWidth',2)
        
        subplot(nSu,1,1)
        title([sprintf('Frequencies %.2f - %.2f Hz',min(min(fuse)),max((max(fuse)))),', \lambda = ',sprintf('%.1f',Lambdas(lambin))]);
        tmx = xlim;
        tmy = ylim;
        set(get(gca,'title'),'Position',[0.3*(tmx(2)-tmx(1)) 1.08*tmy(2) 1.00011])
        set(gca,'FontSize',14)
