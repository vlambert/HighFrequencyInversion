...
outdir = 'SimpleTests/SinglePoint/';
if ~exist(outdir,'dir')
    mkdir(outdir)
end
...
% Load travel times
P_trav = load('P_trav_040_taup.txt');    % TauP with IASP91 velocity model
...
figure(3);clf;
myst = 310;
plot(t,DataFilt(myst,:),'k','LineWidth',2); hold on;
...
%                  mm(la,fi,d,:) = mout(fi,((d-1)*ns+1):(d*ns));
    ...
    info.nDiv = nDiv;
    info.DivPop = DivPop;
    save([outdir,sprintf('InversionOutput_%d.mat',fbin)],'spec','DataSpec','syntmp','specPowerF','mm','GF','Lambdas','fspace','info','-v7.3');
end
