function [Data_pass,Time_pass,sta_lags,sta_pols,stap,nsta_pass] = DoubleTraceXCAlign(DataFilt,sta,xcWin,nsta,tspan,SNR,passCrit,DataS,Time)
% Filtered Data
...
for st=1:nsta
   ...
   Time_tmp = Time(st,:) + lagtime;
   DataFilt(st,:) = interp1(Time(st,:),DataFilt(st,:),Time_tmp,'linear',0);
   Time(st,:) = Time_tmp;
end

% Create second trace with shifted data
trace=zeros(1,nt);
count=0;
for st=1:nsta
   if(max(abs(DataFilt(st,:))) ~= 0 ) % remove flat waveforms and nans
...
for st=1:nsta
   [acor2,lag2] = xcorr(DataFilt(st,tcorr),trace(tcorr),'coeff');
   [ind2,I2] = max(abs(acor));
   corrcoeff(st) = acor2(I2);
   lagDiff = lag2(I2);
   lagtime = lagDiff*dt;
   lags(st) = lagtime;
end
